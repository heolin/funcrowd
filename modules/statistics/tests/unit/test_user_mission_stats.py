...
def test_user_mission_stats_task_data(user1, user2, tasks):
    mission = Mission.objects.first()
    stats = user1.get_mission_stats(mission.id)
    ...
    stats = user2.get_mission_stats(mission.id)
    assert stats.annotated_documents == 0
...
@pytest.mark.django_db
def test_user_mission_stats_annotation_data(tasks_annotations):
    mission = Mission.objects.first()
